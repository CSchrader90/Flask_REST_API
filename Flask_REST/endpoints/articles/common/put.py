...
def put(self, user, article_id):
	...
	if not channel: # if channel doesn't exist, create it
		article_logger.info(f"channel not found in PUT - creating|[article_id:{article_id}, \
	...
	article_logger.info(f"Article updated with channel in PUT|[article_id:{article_id},
																channel:{new_channel_name}")
