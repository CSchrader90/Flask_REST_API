...
def delete(self, user, article_id):
	...
	article_exists = ArticleModel.query.filter_by(user=user.username, article_id=article_id).first()
	...
	delete_article = ArticleModel.query.filter_by(user=user.username, article_id=article_id).first()
